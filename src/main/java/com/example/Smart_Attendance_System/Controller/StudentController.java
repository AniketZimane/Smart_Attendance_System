...
//    StudentRepo studentRepo;
    ...
    public String submitData(Model model, Student stu, MultipartFile file) throws MessagingException, UnsupportedEncodingException {
        ...
        Student studentold=studentRepo.getReferenceById(stu.getEnrollno());
        if(studentold==null)
        {
            String fileNameOld;
...
//        int totalPresenty= attendanceRepo.getTotalPresenty(enrollno,courseId,month,year);
    ...
    @PostMapping("/coursesdept/")
    @ResponseBody
    public List<Course> getCoursesByDeptId(Integer idDept)
    {
        System.out.println("idDept = " + idDept);
        return courseRepo.findByDepartmentId(idDept);
    }
